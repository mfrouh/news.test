...
@section('page-header')
  ...
  <div class="breadcrumb-header justify-content-between">
	<div class="my-auto">
		<div class="d-flex">
			<h4 class="content-title mb-0 my-auto">الكتاب</h4><span class="text-muted mt-1 tx-13 mr-2 mb-0"></span>
		</div>
	</div>
  </div>
  <!-- breadcrumb -->
 		...
 		<div class="card mg-b-20">
 			<div class="card-header pb-0">
 				<div class="d-flex justify-content-between">
 					<h4 class="card-title mg-b-0">الكتاب</h4>
 				</div>
 			</div>
 			<div class="card-body">
 					...
 					<table id="example1" class="table key-buttons text-md-nowrap text-center">
						 ...
						 @foreach ($writers as $writer)
 							<tr>
 								<td>{{$writer->name}}</td>
 								<td>{{$writer->email}}</td>
                                <td>{{$writer->getstatus()}}</td>
                                <td>
                                    @foreach ($writer->categories as $category)
                                         <a href="" class="btn btn-gray-500 btn-sm">{{$category->name}}</a>
                                    @endforeach
                                </td>
                                <td>{{$writer->articles->count()}}</td>
 								<td>
                                    @can('حذف الكاتب من القسم')
